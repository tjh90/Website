...
public class AnascrambleViewModel {
  ...
  private void onClickScrambleBtn() {
    ...
    scrambledComponents =
        letterData.stream().map(l -> new ScrambledCharacter(l.letter(), l.point())).toList();
  ...
  private static CharacterData addOffset(CharacterData letterData, float containerOffset) {
    double letterX = letterData.point().x();
    double letterY = letterData.point().y();
    Point point = new Point(containerOffset + letterX, containerOffset + letterY);
