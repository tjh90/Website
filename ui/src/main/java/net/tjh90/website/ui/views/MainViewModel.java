...
public class MainViewModel {
  ...
  public MainViewModel(final MainView view) {
    ...
    if (darkModeItemElement != null) {
      darkModeItemElement
          .addEventListener(ElementEvents.CLICK, onDarkModeItemClick(darkModeCheckbox))
  ...
  private static DomEventListener onDarkModeItemClick(final Checkbox darkModeCheckbox) {
    return event -> {
      // Ignore clicks originating from the checkbox itself, since those already toggle the checkbox
      // (avoids a double toggle when the click event bubbles up to the menu item).
      boolean isCheckboxClick =
          event.getEventTarget().map(darkModeCheckbox.getElement()::equals).orElse(false);
